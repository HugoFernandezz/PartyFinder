"""
Scraper de eventos para PartyFinder (Versión Playwright + Stealth)
===================================================================
Versión alternativa para Raspberry Pi ARM64 que usa Playwright con 
playwright-stealth para bypassear Cloudflare.
...
async def wait_for_cloudflare(page, timeout: int = 90) -> bool:
    """
    Espera a que se resuelva el challenge de Cloudflare.
    Usa esperas más largas para Raspberry Pi.
    ...
    challenge_keywords = [
        ...
        "please wait", "attention required", "cloudflare", "verifying"
    ]
...
async def scrape_single_venue(url: str, page) -> List[Dict]:
    ...
    try:
        # Navegar a la URL con timeout generoso para ARM64
        await page.goto(url, wait_until="domcontentloaded", timeout=120000)
        
        # Esperar a que pase Cloudflare
        if not await wait_for_cloudflare(page, timeout=90):
            return []
...
async def scrape_all_events(urls: List[str] = None, scrape_tickets: bool = True) -> List[Dict]:
    """
    Scrapea eventos de todas las URLs configuradas usando Playwright + Stealth.
    ...
    # Intentar importar stealth (puede no estar instalado)
    # playwright_stealth v2.0.0 exporta 'stealth' directamente
    have_stealth = False
    stealth_func = None
    try:
        ...
        print("   [INFO] Usando playwright-stealth para evasión de detección")
    except ImportError:
        print("   [WARN] playwright-stealth no instalado, usando Playwright básico")
        print("   [WARN] Instalar con: pip install playwright-stealth")
    
    target_urls = urls or VENUE_URLS
    all_events = []
    
    print("🚀 Iniciando navegador Playwright con stealth...")
    
    try:
        async with async_playwright() as p:
            # Argumentos optimizados para ARM64 / Raspberry Pi
            browser_args = [
                '--no-sandbox',
            ...
            # Aplicar stealth si está disponible (stealth_sync funciona con páginas async)
            if have_stealth and stealth_func:
                stealth_func(page)
...
async def scrape_and_save(upload_to_firebase: bool = False):
    ...
    """
    print("=" * 60)
    print("PartyFinder - Scraper (Playwright + Stealth)")
    print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Scraper de eventos PartyFinder (Playwright + Stealth)',
        epilog='Versión para Raspberry Pi ARM64'
    )
    parser.add_argument(
        '--urls', 
        nargs='+',
